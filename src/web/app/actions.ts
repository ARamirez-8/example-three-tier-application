...
export async function getTasks(): Promise<Task[]> {
  try {
    log('info', 'Fetching tasks from API', { url: `${API_URL}/tasks` });
...
export async function createTask(formData: FormData) {
  try {
    ...
    log('info', 'Creating new task', { title });
    ...
    log('info', 'Task created successfully', { taskId: task.id, title });
    revalidatePath('/');
  } catch (err) {
...
export async function toggleTask(id: number, completed: boolean) {
  try {
    log('info', 'Toggling task completion', { taskId: id, completed });
    ...
    if (!res.ok) {
      log('error', 'Failed to toggle task', { taskId: id, status: res.status });
